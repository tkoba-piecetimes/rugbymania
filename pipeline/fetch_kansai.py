...
def fetch_season(season_year: int) -> dict[str, dict]:
    ...
    for heading, body in sections.items():
        ...
        # Preserve all stages of A/B schedules.
        if not re.search(r'class="sche_cont[78]"', body):
            from legacy_kansai import parse
            matches = parse(body, label, season_year, season_url(season_year))
        else:
            from grid_kansai import parse
            matches = parse(body, label, season_year, season_url(season_year))
        for match in matches:
            match.setdefault("source_url", season_url(season_year))
        if not matches:
            continue
        result[code] = {
            "matches": matches,
            ...
            "label": label,
            "source_url": season_url(season_year),
        }
...
def main() -> None:
    ...
    sync(fetch_season, [CURRENT_SEASON] + HISTORY_SEASONS, CURRENT_SEASON, "関西", "関西ラグビーフットボール協会")
