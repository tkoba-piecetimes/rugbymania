...
pipeline/generate_articles.py を移植・ラグビーのデータ構造に適合）。

生成する記事タイプは2種類:

  1. 結果まとめ（review-<league>-<YYYYMMDD>）
     当該リーグの「試合日クラスタ」（開催日が近い試合をまとめた週末単位、
     WEEKEND_GAP_DAYS以内なら同一クラスタ）の結果をまとめる。対象は関東（対抗戦A/B・
     リーグ戦1/2部）・関西（A/Bリーグ）・九州（A〜Dリーグ）の全10リーグ共通。
     ...
     【2026-08時点の注記】2026シーズンは関東・関西とも9月開幕（九州は未発表）で、
     現時点でstatus=playedの試合はリーグ全体で0件。従って本タイプの候補は
     シーズン開幕まで0件が続く（fetch_all.pyが日次で結果を取得し次第、
     自動的に候補が生まれる。このスクリプト自体の変更は不要）。
  ...
  2. チームシーズン記録（season-<league>-<team_slug>-<year>）
     ...
     2026-08時点でhistory/を持つのはkanto-taiko-a/b・kanto-league-1/2（2023〜2025年）、
     kansai-a（2024〜2025年）・kansai-b（2025年のみ）、kyushu-a/b/c（2023〜2025年）、
     kyushu-d（2023年のみ）。2026シーズンが未開幕の間、このタイプが在庫の主力になる。
